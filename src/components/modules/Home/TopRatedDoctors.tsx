...
const DoctorCard = ({ doctor }: { doctor: (typeof doctors)[0] }) => {
  return (
    <Card className="text-center overflow-hidden hover:shadow-xl transition-shadow duration-300">
      <CardHeader className="bg-orange-100 items-center p-6">
        <Image
          src={doctor.image}
          alt={doctor.name}
          width={96}
      ...
      <CardFooter className="grid grid-cols-2 gap-2 p-4 pt-0">
        <Link
          href={'/my-profile'}
          className="border-chart-1 border text-background font-bold"
        >
          View Profile
        </Link>
        <Link href={'/patient-dashbaord'} className="bg-foreground text-muted">
          Book Now
        </Link>
      </CardFooter>
    </Card>
